//********************************************************************************************************************************
//*                                                                                                                              *
//*     ID3v2 Library 2.0.45.103 © 3delite 2010-2016                                                                             *
...
Const
    ID3V2LIBRARY_VERSION = $020045103;
...
Const
    ...
    function ReverseBytes16(ASmallInt: SmallInt): SmallInt; register;
...
var
  ...
  ReadBytesLong, MemoryStreamPosition: Int64;
  I: Integer;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Stream.Read(MagicBytes[0], SizeOf(MagicBytes));
        if (MagicBytes[0] = ORD('P')) and
           (MagicBytes[1] = ORD('R')) and
           (MagicBytes[2] = ORD('O')) and
           (MagicBytes[3] = ORD('P')) then
        begin
          Stream.Read(ReadBytesLong, 8);
          ReadBytesLong := ReverseBytes64(ReadBytesLong);
          //MemoryStreamPosition := Stream.Position;
          MemoryStream.CopyFrom(Stream, ReadBytesLong);
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                MemoryStream.Read(SampleRate, 4);
              ...
              begin
                MemoryStream.Read(ReadBytesLong, 8);
                ReadBytesLong := ReverseBytes64(ReadBytesLong);
                MemoryStreamPosition := MemoryStream.Position;
                MemoryStream.Read(ChannelNumber, 2);
                ChannelNumber := ReverseBytes16(ChannelNumber);
                MemoryStream.Seek(MemoryStreamPosition, soBeginning);
                MemoryStream.Seek(ReadBytesLong, soCurrent);
              end
              else if (MagicBytes[0] = ORD('C')) and
...
//                MemoryStream.Read(DFFInfo.SampleCount, 4);
//                TDFF.SampleCount := ReverseBytes(TDFF.SampleCount);
//                MemoryStream.Seek(MemoryStreamPosition, soBeginning);
                MemoryStream.Seek(ReadBytesLong, soCurrent);
              end
...
begin
    ...
    try
        Stream.Read(MajorVersion, 1);
        Stream.Read(MinorVersion, 1);
    except
        Exit;
    end;
    if (MajorVersion > 4)
    ...
    OR (Stream.Position >= FPosition + 10 + Self.Size);
    Result := ID3V2LIBRARY_SUCCESS;
    Loaded := True;
end;

function TID3v2Tag.LoadFromStream(Stream: TStream): Integer;
var
    PreviousPosition: Int64;
begin
    Loaded := False;
    try
            ...
            if CheckRIFF(Stream) then begin
                ...
                if CheckRF64(Stream) then begin
                    ...
                    if CheckAIFF(Stream) then begin
                        ...
                        if ParseAudioAttributes then begin
                            AIFFInfo := GetAIFFInformation(Stream);
                        end;
        ...
        Result := LoadTagFromStream(Stream);
    finally
        //* Check if source is an MPEG
        Stream.Seek(Self.Size, soBeginning);
        if CheckMPEG(Stream) then begin
            FSourceFileType := sftMPEG;
            if ParseAudioAttributes then begin
                //Stream.Seek(Self.Size, soBeginning);
                MPEGInfo := MPEGProcessHeader(Stream);
                Self.BitRate := MPEGInfo.BitRate;
                //Stream.Seek(Self.Size, soBeginning);
                GetMPEGHeaderInformation(Stream);
            end;
        end;
    end;
end;
...
begin
    try
        Stream.Read(ExtendedFrameID[0], 4);
        ...
        if NOT ValidID3v2FrameID(ExtendedFrameID) then begin
            ...
            if MajorVersion = 3 then begin
                with ExtendedHeader3 do begin
                    ...
                    Stream.Read(ExtendedHeader3.Flags, 2);
...
begin
    try
        ...
        if MajorVersion = 2 then begin
            ...
            Flags := 0;
            Stream.Read(Size, 3);
            Frames[FrameIndex].Size := ReverseBytes32(Size SHL 8);
            if (Frames[FrameIndex].Size < 1)
            OR (Frames[FrameIndex].Size > Self.Size)
            then begin
                Exit;
        ...
        if MajorVersion = 3 then begin
            Stream.Read(Size, 4);
            Stream.Read(Flags, 2);
            Frames[FrameIndex].Size := ReverseBytes32(Size);
            if (Frames[FrameIndex].Size < 1)
            OR (Frames[FrameIndex].Size > Self.Size)
            then begin
                Exit;
            end;
            Frames[FrameIndex].Flags := ReverseBytes16(Flags);
            Frames[FrameIndex].DecodeFlags3;
            Frames[FrameIndex].Unsynchronised := Unsynchronised;
            if Frames[FrameIndex].Compressed then begin
                Stream.Read(DataLengthIndicatorValueCoded, 4);
            ...
            if Frames[FrameIndex].Encrypted then begin
                Stream.Read(Frames[FrameIndex].EncryptionMethod, 1);
                Frames[FrameIndex].Size := Frames[FrameIndex].Size - 1;
            end;
            if Frames[FrameIndex].GroupingIdentity then begin
                Stream.Read(Frames[FrameIndex].GroupIdentifier, 1);
                Frames[FrameIndex].Size := Frames[FrameIndex].Size - 1;
            end;
            Frames[FrameIndex].Stream.CopyFrom(Stream, Frames[FrameIndex].Size);
        end;
                    ...
                    if CheckAIFF(Stream) then begin
                        ...
                        if CheckRF64(Stream) then begin
                            ...
                            if CheckDSF(Stream) then begin
                                ...
                                Stream.Seek(0, soBeginning);
                                if CheckDFF(Stream) then begin
                                    if NOT ValidDFF(Stream) then begin
                                        Result := ID3V2LIBRARY_ERROR_CORRUPT;
                                        Exit;
                                    end;
                                    Result := SaveDFF(Stream, WriteTagTotalSize);
                                    Exit;
                                end else begin
                                    //* Normal file (MP3) - tag at start
                                    Stream.Seek(0, soBeginning);
        ...
        case Data of
            ...
            1: begin
                ...
                repeat
                    ...
                    AND (Frames[FrameIndex].Stream.Position <> Frames[FrameIndex].Stream.Size)
                    then begin
                        SetLength(Bytes, Length(Bytes) + 4);
                        ...
                        if DataWord <> 0 then begin
                            SetLength(Bytes, Length(Bytes) + 2);
                            Bytes[ByteCounter] := Byte(DataWord);
                            Inc(ByteCounter);
    ...
    try
        for i := 0 to FrameCount - 1 do begin
            ...
            if MajorVersion = 3 then begin
                ...
                then begin
                    Stream.Write(Frames[i].DataLengthIndicatorValue, 4);
                end;
                if Frames[i].Encrypted then begin
        ...
        then begin
            ...
            while Stream.Position + 8 < Stream.Size do begin
                ...
                then begin
                    Stream.Seek(- 4, soCurrent);
                    PreviousPosition := Stream.Position;
                    Stream.Seek(ChunkSize + 4, soCurrent);
                    if Stream.Position < Stream.Size then begin
                        if FileName <> '' then begin
                            try
                                TempStream := TFileStream.Create(ChangeFileExt(FileName, '.tmp'), fmCreate);
...
begin
    ...
    try
        ...
        Stream.Seek(4, soBeginning);
        Stream.Read(DFFFRM8Size, 8);
        DFFFRM8Size := ReverseBytes64(DFFFRM8Size);
        Dec(DFFFRM8Size, ExistingTagSizePadded + 12);
        Stream.Seek(- 8, soCurrent);
        DFFFRM8Size := ReverseBytes64(DFFFRM8Size);
...
begin
    ...
    try
        ...
        if Padded then begin
            Inc(DFFFRM8Size, WriteTagTotalSize + 1);
        end else begin
